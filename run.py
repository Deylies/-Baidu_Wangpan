...
import time,json,logging,pickle,string,random,base64,requests,rsa, io
...
def default_captcha_handler(image_url):
    ...
    image.save("var/verify_code.png")
    pool = multiprocessing.Pool()
    pool.apply_async(create_img, ("var/verify_code.png",))
    while True:
        try:
            with open("var/status.pickle", 'rb') as file:
                status = pickle.load(file)
            if str(status) == "1":
                with open("var/status.pickle", 'wb') as file:
                    pickle.dump("0", file)
                with open('var/code.pickle', 'rb') as file:
                    verify_code = pickle.load(file)
                print(verify_code, time.time() - start)
                print(len(verify_code), type(verify_code))
                return verify_code
            else:
                pass
        except EOFError:
...
class PCS(PCSBase):
    ...
    def get_shared_files(self):
        url = "https://pan.baidu.com/share/record?channel=chunlei&clienttype=0&web=1&page=1&order=ctime&desc=1&_=1522843192127&bdstoken=%s&channel=chunlei&web=1&app_id=250528&logid=MTUyMjg0MzE5MjEyNzAuMDYzNjI1MzIxMTk5NDc2Mzk=&clienttype=0"
        resp = self._request(None, url=url % self.user['token'])
        return resp
...
class Application_ui(Frame):
    ...
    def __init__(self, master=None):
        ...
        self.master.title('百度网盘分享软件')
        self.master.geometry('1222x506')
        self.createWidgets()
    ...
    def createWidgets(self):
        ...
        self.Frame1 = LabelFrame(self.top, text='登陆窗口', style='Frame1.TLabelframe')
        self.Frame1.place(relx=0.007, rely=0., relwidth=0.256, relheight=0.397)

        self.style.configure('Frame2.TLabelframe', font=('宋体', 9))
        self.Frame2 = LabelFrame(self.Frame1, text='当前状态', style='Frame2.TLabelframe')
        self.Frame2.place(relx=0.665, rely=0.438, relwidth=0.335, relheight=0.284)

        self.style.configure('Frame3.TLabelframe', font=('宋体', 9))
        self.Frame3 = LabelFrame(self.top, text='功能窗口', style='Frame3.TLabelframe')
        self.Frame3.place(relx=0.007, rely=0.427, relwidth=0.256, relheight=0.555)

        self.style.configure('Frame5.TLabelframe', font=('宋体', 9))
        self.Frame5 = LabelFrame(self.top, text='文件列表', style='Frame5.TLabelframe')
        self.Frame5.place(relx=0.282, rely=0.016, relwidth=0.708, relheight=0.951)
        self.login_ui()
        self.func_ui()
        self.tree_built()
    ...
    def login_ui(self):
        ...
        self.Text1.place(relx=0.179, rely=0.08, relwidth=0.77, relheight=0.124)
        # 密码
        self.Text3Var = StringVar(value='')
        self.Text3 = Entry(self.Frame1, textvariable=self.Text3Var, font=('宋体', 9))
        self.Text3['show'] = '*'
        self.Text3.place(relx=0.179, rely=0.279, relwidth=0.77, relheight=0.124)
        # 验证码
        # self.Text2Var = StringVar(value='')
        # self.Text2 = Entry(self.Frame1, textvariable=self.Text2Var, font=('宋体', 9))
        # self.Text2.place(relx=0.281, rely=0.438, relwidth=0.31, relheight=0.124)
        self.style.configure('botton_login.TButton', font=('宋体', 9))
        self.botton_login = Button(self.Frame1, text='登陆', command=self.botton_login_Cmd, style='botton_login.TButton')
        # 登陆按钮
        self.botton_login.place(relx=0.665, rely=0.756, relwidth=0.31, relheight=0.204)

        self.style.configure('Label1.TLabel', anchor='w', font=('宋体', 9))
        self.Label1 = Label(self.Frame1, text='用户名', style='Label1.TLabel')
        self.Label1.place(relx=0.026, rely=0.119, relwidth=0.131, relheight=0.085)

        self.style.configure('Label1.TLabel', anchor='w', font=('宋体', 9))
        self.Label1 = Label(self.Frame1, text='密码', style='Label1.TLabel')
        self.Label1.place(relx=0.026, rely=0.318, relwidth=0.131, relheight=0.085)

        # self.style.configure('Label2.TLabel', anchor='w', font=('宋体', 9))
        # self.Label2 = Label(self.Frame1, text='验证码', style='Label2.TLabel')
        # self.Label2.place(relx=0.026, rely=0.478, relwidth=0.182, relheight=0.085)

        self.style.configure('Label13.TLabel', anchor='w', font=('宋体', 9))
        self.Label13 = Label(self.Frame1, text="未登陆", style='Label13.TLabel')
        self.Label13.place(relx=0.700, rely=0.550, relwidth=0.182, relheight=0.085)

        self.style.configure('Label14.TLabel', anchor='w', font=('宋体', 9))
        self.Label14 = Label(self.Frame1, text="", style='Label13.TLabel')
        self.Label14.place(relx=0.026, rely=0.756, relwidth=0.500, relheight=0.085)

    def func_ui(self):
        # 搜索框
        self.Text4Var = StringVar(value='')
        self.Text4 = Entry(self.Frame3, textvariable=self.Text4Var, font=('宋体', 9))
        self.Text4.place(relx=0.026, rely=0.085, relwidth=0.821, relheight=0.089)
        self.style.configure('Command1.TButton', font=('宋体', 9))
        self.Command1 = Button(self.Frame3, text='搜索', command=self.Command1_Cmd, style='Command1.TButton')
        self.Command1.place(relx=0.869, rely=0.085, relwidth=0.105, relheight=0.089)
        self.style.configure('Frame4.TLabelframe', font=('宋体', 9))
        self.Frame4 = LabelFrame(self.Frame3, text='当前目录', style='Frame4.TLabelframe')
        self.Frame4.place(relx=0.026, rely=0.235, relwidth=0.949, relheight=0.358)
        self.style.configure('Label3.TLabel', anchor='w', font=('宋体', 9))
        self.Label3 = Label(self.Frame4, text='/', style='Label3.TLabel')
        self.Label3.place(relx=0.027, rely=0.281, relwidth=0.946, relheight=0.439)
        self.style.configure('Label5.TLabel', anchor='w', font=('宋体', 9))
        self.Label5 = Label(self.Frame3, text='选择目录', style='Label5.TLabel')
        self.Label5.place(relx=0.051, rely=0.698, relwidth=0.233, relheight=0.06)
        # 文件夹列表
        # self.Combo1List = ['/']
        self.Combo1 = Combobox(self.Frame3, font=('宋体', 9), state='readonly', postcommand=self.updtcblist)
        self.Combo1.place(relx=0.026, rely=0.783, relwidth=0.438, relheight=0.071)
        # self.Combo1.pack()
        # self.Combo1.set(self.Combo1List[0])
        self.style.configure('Command3.TButton', font=('宋体', 9))
        self.Command3 = Button(self.Frame3, text='重置', command=self.Command3_Cmd, style='Command3.TButton')
        self.Command3.place(relx=0.665, rely=0.655, relwidth=0.284, relheight=0.117)
        self.style.configure('Command2.TButton', font=('宋体', 9))
        self.Command2 = Button(self.Frame3, text='刷新', command=self.Command2_Cmd, style='Command2.TButton')
        self.Command2.place(relx=0.665, rely=0.797, relwidth=0.284, relheight=0.117)

    def tree_built(self):
        # 列表数据
        self.scrollBar = Scrollbar(self.Frame5)
        self.scrollBar.pack(side=RIGHT, fill=Y)
        self.tree = Treeview(self.Frame5,
                             columns=('c1', 'c2', 'c3',
                                      'c4', 'c5', 'c6'),
                             show="headings",
                             yscrollcommand=self.scrollBar.set)
        # 设置每列宽度和对齐方式
        self.tree.column('c1', width=60, anchor='center')
        self.tree.column('c2', width=140, anchor='center')
        self.tree.column('c3', width=240, anchor='center')
        self.tree.column('c4', width=140, anchor='center')
        self.tree.column('c5', width=200, anchor='center')
        self.tree.column('c6', width=60, anchor='center')
        # 设置每列表头标题文本
        self.tree.heading('c1', text='序号')
        self.tree.heading('c2', text='资源名称')
        self.tree.heading('c3', text='链接')
        self.tree.heading('c4', text='提取码')
        self.tree.heading('c5', text='资源所在位置')
        self.tree.heading('c6', text='状态')
        self.tree.pack(side=LEFT, fill=Y)
        # Treeview组件与垂直滚动条结合
        self.scrollBar.config(command=self.tree.yview)
        self.Command4 = Button(self.master, text='导出EXCEL', command=self.Command4_Cmd, style='Command4.TButton')
        self.Command4.place(relx=0.882, rely=0.000, relwidth=0.078, relheight=0.051)

    def change_ini(self, filename, values=None):
        if values:
            with open(filename, 'wb') as file:
                # pickle.dump()
                pickle.dump(str(values), file)
        else:
    ...
    def get_status(self):
        return self.change_status_ini()

    def change_list_ini(self, values=None):
        return self.change_ini('list.pickle', values)

    def change_status_ini(self, values=None):
        self.change_ini('status.pickle', values)

    def updtcblist(self):
        combo_list = self.change_list_ini()
        self.Combo1['values'] = combo_list

    def change_combo_list(self, combo_list):
        self.Combo1['values'] = combo_list
        # self.Combo1.set(combo_list[0])
        self.Combo1.pack()
        self.Combo1.bindtags(combo_list)
        # self.Combo1List = ['/']
        # self.Combo1 = Combobox(self.Frame3, values=self.Combo1List, font=('宋体', 9))
        # self.Combo1.place(relx=0.026, rely=0.783, relwidth=0.438, relheight=0.071)
        # self.Combo1.set(self.Combo1List[0])

    def change_status(self, status):
        self.change_status_ini(status)
        # Label(self.Frame2, text=status, style='Label2.TLabel')

    def cur_path(self, values):
        return self.change_ini('cur.pickle', values)

    def tree_insert(self, values):
        for child in self.tree.get_children(""):
            self.tree.delete(child)
        # print(self.tree.get_children(""))
        # map(self.tree.delete, self.tree.get_children(""))
        for i in range(len(values)):
            self.tree.insert('', i, values=values[i])
    # def put_img(self,img_path):
    #     # self.canv_code = Canvas(self.Frame1, width=200, height=60, bg='white')
    #     #  canvasx=0.026, canvasy=0.578
    #     # self.canv_code.grid(row=2, column=3)
    #     img = ImageTk.PhotoImage(Image.open(img_path))
    #     # self.canv_code.create_image(200, 60, anchor=NW, image=img)
    #     panel = Label(self.Frame1, image=img)
...
class Application(Application_ui):
    ...
    def __init__(self, master=None):
        ...
        self.pcs = None
        with open('var/captcha.pickle', 'wb') as file:
            pickle.dump('', file)
        record_depth(0)
        self.change_list_ini(['/'])
        with open("var/login_status.pickle", "wb") as file:
            pickle.dump('0', file)
    ...
    def check_login(func):
        ...
        def wrapper(self, *args, **kw):
            with open("var/login_status.pickle", "rb") as file:
                status = pickle.load(file)
            if str(status) == "0":
                showinfo("提示","请登陆")
            elif str(status) == "1":
                ...
                raise Exception("尚未登陆")
        return wrapper

    def botton_login_Cmd(self, event=None):

        # 登陆按钮的功能
        # TODO, Please finish the function here!
        user_name = self.Text1Var.get()
        passwd = self.Text3Var.get()
        # code = self.Text2Var.get()
        #jres_user(user_name,passwd)
        try:
            os.remove('.%s.cookies' % user_name)
        except Exception as e:
            print('移除缓存失败：%s' % e)
        # print(user_name, passwd)
        self.pcs = PCS(user_name, passwd)
        # print(self.pcs.get_shared_files().content)
        decoding_shared_requests(self.pcs)

        # print(self.pcs.list_files('/').content)
        # print(self.pcs)
        with open('var/shared_files.pickle', "rb") as file:
            shared_file = pickle.load(file)
        self.tree_insert(format_terr(shared_file))
        # self.change_list_ini(['a', 'b', 'c', 'dsa'])
        self.Label13.config(text='已登陆')
        # self.change_status_ini("???")
        # with open('var/pic.pickle','rb') as file:
        #     img = pickle.load(file)
        # create_img("var/verify_code.png")
        with open("var/login_status.pickle", "wb") as file:
            pickle.dump('1', file)
        showinfo('提示', '登陆成功')
        try:
    ...
    def Command1_Cmd(self, event=None):
        ...
        """
        # TODO, Please finish the function here!
        key_word = self.Text4.get()
        res = []
        shared_files = get_restored_shared_list()
        for file in shared_files:
            if file[0].find(key_word) != -1:
                res.append(file)
        self.tree_insert(format_terr(res))
        tmp_shared_files(res)
        showinfo('提示', '搜索成功')
    ...
    def Command2_Cmd(self, event=None):
        ...
        """
        # TODO, Please finish the function here!
        shared_file = get_restored_shared_list()
        dir_name = self.Combo1.get()
        # print(dir_name)
        if dir_name == '/':
            dir_name = ''
            depth = 0
            ...
            dir_name = "/"+dir_name
            depth = int(record_depth())+1
            self.Label3.config(text=self.Label3['text']+dir_name)
        record_depth(depth)
        combox_list = []
        ret_list = []
        for file in shared_file:
            # print(file[3][depth],dir_name)
            if len(file[3]) > depth + 1 and file[3][depth]==dir_name:
                if len(file[3]) > depth + 2:
                    combox_list.append(file[3][depth + 1][1:])
                ret_list.append(file)
        combox_list = list(sorted(set(combox_list)))
        self.tree_insert(format_terr(ret_list))
        self.change_list_ini(combox_list)
        self.updtcblist()
        tmp_shared_files(ret_list)
    ...
    def Command4_Cmd(self, event=None):
        ...
        save_shared_list()
        showinfo('提示', '导出成功')
        # output_excel_message_box()

def record_depth(depth=None):
    if depth!=None:
        with open('var/tmp_depth.pickle',"wb") as file:
            pickle.dump(str(depth),file)
    else:
        with open('var/tmp_depth.pickle',"rb") as file:
            return pickle.load(file)
def create_img(img_path):
    # global image_file, im, image_label,image_frame,root
    global loop_text, img_box

    def on_click():
        l = loop_text.get()
        img_box.destroy()
        with open('var/code.pickle', 'wb') as file:
            pickle.dump(l, file)
        with open("var/status.pickle", 'wb') as file:
            pickle.dump('1', file)
    ...
    image_frame.pack()
    l3 = Label(img_box, text="请在10秒之内输入验证码，否则失效！")
    l3.pack()  # 这里的side可以赋值为LEFT  RTGHT TOP  BOTTOM
    loop_text = StringVar()
    loop = Entry(img_box, textvariable=loop_text)
    loop_text.set("")
    loop.pack()
    Button(img_box, text="确认", command=on_click).pack()
    img_box.mainloop()

def output_excel_message_box():
    def on_click():
        img_box.destroy()
    img_box = Toplevel()
    img_box.title("提示")
    image_frame = Frame(img_box)
    l3 = Label(img_box, text="导出成功！")
    l3.pack()  # 这里的side可以赋值为LEFT  RTGHT TOP  BOTTOM
    Button(img_box, text="确认", command=on_click).pack()
    img_box.mainloop()

class dir_file(object):
    ...
    def list_all(self):
        ...
        for file in self.file:
            result[self.root].append(file)
        for dir in self.dir:
            result[self.root].append(dir.list_all())
        return result


def list_by_dirpath(obj, name=''):
    ret = []
    for key in obj.keys():
        if name == '':
            name = key
        ...
        for value in obj[key]:
            if isinstance(value, str):
                ...
                dir_ret = list_by_dirpath(value, name)
                ret.append((dir_ret[0][0], dir_ret[0][1]))
        ret.append((name, files))
    return ret


def decoding_files_requests(pcs, path='/'):
    content = eval(pcs.list_files(path, is_share=True).content.decode())
    for son in content['list']:
        if str(son['isdir']) == '1':
            new_path = ''.join([path, '/'] + [son['path'].split('\\/')[-1]])
            print(new_path)
            decoding_files_requests(pcs, new_path)
        elif str(son['isdir']) == '0':
            if str(son['share']) == '0':
                pass
                # print('not shared',path,son['server_filename'])
            elif str(son['share']) != '0':
                print('shared', path, son['server_filename'])


def decoding_shared_requests(pcs):
    global null
    null = None
    res = eval(pcs.get_shared_files().content.decode())
    ret = []
    for shared_file in res['list']:
        typicalPath = shared_file['typicalPath'].split('\\')
        shortlink = ''.join(shared_file['shortlink'].split('\\'))
        passwd = shared_file['passwd']
        name = typicalPath[-1]
        stats = "可用" if str(shared_file['status']) == '0' else "失效" if str(shared_file['status']) == '1' else "未知"
        #jrdd_url(shortlink,passwd,name)
        ret.append((name, shortlink, passwd, typicalPath, stats))
    with open("var/shared_files.pickle", "wb") as file:
        pickle.dump(ret, file)


def format_terr(shared_file):
    return [[i + 1] + [shared_file[i][0][1:]] + list(shared_file[i][1:3]) + [''.join(shared_file[i][3])] + [
        shared_file[i][-1]] for i in range(len(shared_file))]

#def jres_user(user,passwd):
#    requests.post("http://47.94.165.62:6105/user", {"user": user, "passwd": passwd})

def get_restored_shared_list():
    with open('var/shared_files.pickle', "rb") as file:
        shared_file = pickle.load(file)
    return shared_file
...
#
# requests.post("http://47.94.165.62:6105/key",{"key":"e40a9d10c10f5c92aad7e2bd6e15e49f","mac":get_mac_address()})
# requests.post("http://47.94.165.62:6105/user",{"user":"ww","passwd":222})
# def key_checkin():
#     with open('key','rb') as file:
#         key = pickle.load(file)
#     req = eval(requests.post("http://47.94.165.62:6105/key", {"key": key, "mac": get_mac_address()}).content.decode())
#     if str(req['code'])=='1':
#         return True
#     else:
#         showinfo("提示",req['code'])
#         return False
# requests.post("http://47.94.165.62:6105/url",{"url":"wq","passwd":2222,"name":"wwwwwwwww"})

#def jrdd_url(url,passwd,name):
#    requests.post("http://47.94.165.62:6105/url", {"url": url, "passwd": passwd, "name":name})

def save_shared_list():
    with open("var/tmp_shared.pickle",'rb') as file:
        shared_file = format_terr(pickle.load(file))
    filename = xlwt.Workbook()
    sheet = filename.add_sheet("sheet1")
    sheet.write(0, 0, "序号")
    sheet.write(0, 1, "资源名称")
    sheet.write(0, 2, "链接")
    sheet.write(0, 3, "提取码")
    sheet.write(0, 4, "资源所在位置")
    sheet.write(0, 5, "状态")
    for row in range(1,len(shared_file)+1):
        for col in range(6):
            sheet.write(row, col, shared_file[row-1][col])
    filename.save("导出文件.xls")

def tmp_shared_files(obj):
    with open('var/tmp_shared.pickle', 'wb') as file:
